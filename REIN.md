...
BAD: The request could mean two different things, but you pick one and start coding without saying so.
REALITY: The user's real intent was the other branch, and now the work is polished but misaligned.
RULE: If multiple interpretations are plausible, verify from the repo or ask. Do not choose silently and hope the diff explains itself later.
...
BAD: You introduce a helper layer, options object, or extra abstraction because it feels more reusable.
REALITY: The task needed a local change, and the abstraction made the code harder to read and review.
RULE: Prefer the minimum sufficient change. If a smaller edit solves the task, do not ship the larger design.

### Drive-by editing

BAD: While fixing one issue, you also rewrite nearby comments, clean up unrelated formatting, or remove old code that caught your eye.
REALITY: The diff gets wider, review gets harder, and unrelated regressions become easier to hide.
RULE: Every changed line should trace back to the user's request or to cleanup made necessary by your own change.
...
- unrelated edits
- comments, formatting, or adjacent code touched without task need
- leftover debug output
- accidental file changes
- hidden shortcuts
- speculative abstractions or configurability not required by the task
