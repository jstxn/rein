...
function numericOwnValue(values, key) {
...
export function dotVector(buffer, vectorIndex, queryVector) {
  ...
  for (let index = 0; index < VECTOR_DIMENSIONS; index += 1) {
    const indexedValue = buffer.readFloatLE(start + index * 4);
    const queryValue = queryVector[index];
    if (Number.isFinite(indexedValue) && Number.isFinite(queryValue)) {
      score += indexedValue * queryValue;
    }
  }
...
export function documentFrequencies(chunks) {
  ...
  for (const chunk of chunks) {
    for (const term of Object.keys(chunk.terms)) {
      frequencies[term] = numericOwnValue(frequencies, term) + 1;
    }
  }
  return frequencies;
}
...
export function bm25Score(chunk, queryTerms, frequencies, totalChunks, averageLength) {
  ...
  for (const term of Object.keys(queryTerms)) {
    const tf = numericOwnValue(chunk.terms, term);
    if (tf === 0) {
      continue;
    }

    const df = numericOwnValue(frequencies, term);
    const idf = Math.log(1 + (totalChunks - df + 0.5) / (df + 0.5));
    const termCount =
      typeof chunk.termCount === "number" && Number.isFinite(chunk.termCount) ? chunk.termCount : 0;
    const normalizedAverageLength =
      typeof averageLength === "number" && Number.isFinite(averageLength) && averageLength > 0
        ? averageLength
        : 1;
    const denominator = tf + k1 * (1 - b + b * (termCount / normalizedAverageLength));
