...
export async function resolveInterviewSlug(slug, actionLabel, cwd) {
  if (slug) {
    return slug;
  }

  const interviews = listInterviewStates({ cwd });
  if (interviews.length === 0) {
    throw new Error("No interview state files were found under .rein/state.");
  }
