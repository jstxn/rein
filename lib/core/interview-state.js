...
import { formatInterviewStatusFrame } from "./interview-presentation.js";
...
function normalizeProfile(profile) {
  ...
  if (!PROFILE_CONFIG[normalized]) {
    throw new ReinError(`Unknown profile: ${profile}`);
  }
...
function normalizeContextType(contextType) {
  ...
  if (!WEIGHTS[normalized]) {
    throw new ReinError(`Unknown context type: ${contextType}`);
  }
...
function normalizeDimensionScores(rawScores, contextType) {
  ...
  for (const dimension of Object.keys(weights)) {
    ...
    if (typeof value !== "number" || Number.isNaN(value) || value < 0 || value > 1) {
      throw new ReinError(`Score for "${dimension}" must be a number between 0 and 1.`);
    }
...
function normalizeTargetDimension(target, contextType) {
  ...
  if (!normalized) {
    throw new ReinError("Interview rounds require a target dimension.");
  }
...
export function initInterviewState(options = {}) {
  ...
  if (!idea) {
    throw new ReinError("Interview init requires an idea.");
  }

  const slug = options.slug ? slugify(options.slug) : slugify(idea);
  if (!slug) {
    throw new ReinError("Unable to derive a slug from the provided idea.");
  }
  ...
  if (fs.existsSync(paths.state) && !options.force) {
    throw new ReinError(
      `Interview state already exists for slug "${slug}". Use --slug or --force.`,
    );
  }
...
export function loadInterviewState(slug, options = {}) {
  ...
  if (!state) {
    throw new ReinError(`No interview state found for slug "${slug}".`);
  }
...
export function updateInterviewRound(options = {}) {
  ...
  if (state.status === "completed") {
    throw new ReinError(`Interview "${state.slug}" is already completed.`);
  }

  const expectedRound = state.currentRound + 1;
  if (options.round && Number(options.round) !== expectedRound) {
    throw new ReinError(`Expected round ${expectedRound}, received ${options.round}.`);
  }
  ...
  if (!question || !answer) {
    throw new ReinError("Interview rounds require target, question, and answer values.");
  }
...
function validateCrystallizeSummary(summary) {
  const missingStrings = REQUIRED_SUMMARY_STRING_FIELDS.filter((field) => {
    return typeof summary[field] !== "string" || summary[field].trim().length === 0;
  });
  const missingArrays = REQUIRED_SUMMARY_ARRAY_FIELDS.filter((field) => {
    return !Array.isArray(summary[field]) || summary[field].length === 0;
  });

  const missing = [...missingStrings, ...missingArrays];
  if (missing.length > 0) {
    throw new ReinError(
      `Crystallize summary JSON must include non-empty fields: ${missing.join(", ")}.`,
    );
  }
}
...
export function crystallizeInterview(options = {}) {
  ...
  validateCrystallizeSummary(summary);
...
export function formatInterviewStatus(view) {
  return formatInterviewStatusFrame(view);
}
