...
const LEGACY_SKILL_RENAMES = {
  "rein-review": ["rein-diff-review"],
};

const TOOL_NAMES = ["codex", "claude", "cursor"];
...
function toolRootDir(tool) {
  return TOOL_ROOT_DIRS[tool] || TOOL_ROOT_DIRS.codex;
}

function toolSkillSurfacePath(baseDir, tool, skill) {
  if (tool === "codex") {
    return path.join(baseDir, ".codex", "skills", skill);
  }

  if (tool === "claude") {
    return path.join(baseDir, ".claude", "commands", `${skill}.md`);
  }

  return path.join(baseDir, ".cursor", "rules", `${skill}.mdc`);
}

function detectInstalledToolsIn(baseDir) {
  return TOOL_NAMES.filter((tool) => exists(path.join(baseDir, toolRootDir(tool), "rein-install")));
}
...
function usage() {
  return [
    ...
    `  ${bold("Usage")}`,
    `    ${dim("$")} rein init`,
    `    ${dim("$")} rein init --repo [path] [--codex] [--claude] [--cursor] [--link]`,
    `    ${dim("$")} rein init --user [--codex] [--claude] [--cursor] [--link]`,
    `    ${dim("$")} rein status [--repo path] [--user] [--codex] [--claude] [--cursor]`,
    `    ${dim("$")} rein update [--repo path] [--user] [--codex] [--claude] [--cursor] [--link]`,
    `    ${dim("$")} rein remove [--repo path] [--user] [--yes] [--codex] [--claude] [--cursor]`,
    "",
    `  ${bold("Commands")}`,
    ...
    `    ${bold("--cursor")}  Target Cursor (.cursor/rules/)`,
...
function createSymlink(src, dest, kind) {
  ...
  const linkType = process.platform === "win32" && kind === "dir" ? "junction" : kind;
...
function missingPackagedAssets() {
  const requiredPaths = [
    ...
    ...REPO_SKILLS.map((skill) => path.join(".cursor", "rules", `${skill}.mdc`)),
  ];
...
### Workflow
- use \`rein-go\` when you want REIN to carry one task through clarification, planning, implementation, cleanup, diff review, and verification
...
function writeInstallNotes(installNotesDir, installedFrom, installedInto, force, installMode) {
...
function readInstallMetadata(targetDir, tool) {
  const notesFile = path.join(targetDir, toolRootDir(tool), "rein-install", "installed-from.txt");
  if (!exists(notesFile)) {
    return null;
  }
...
function summarizeInstallMode(targetDir, tools) {
  const modes = tools.map((tool) => readInstalledMode(targetDir, tool) || "copy");
...
function resolveInstalledMode(targetDir, tools) {
  ...
  if (uniqueModes.length > 1) {
    throw new ReinError(
      "Detected mixed REIN install modes. Re-run with --codex, --claude, or --cursor, or pass --link to normalize the install.",
    );
  }
...
function addLinkModeNextStep(nextSteps, installMode) {
  ...
  nextSteps.push(`Linked installs depend on ${REPO_ROOT} remaining available at the same path`);
}

function detectInstalledToolsUser(home = os.homedir()) {
  return detectInstalledToolsIn(home);
}

function detectInstalledTools(targetDir) {
  return detectInstalledToolsIn(targetDir);
}

function resolveTools(parsed) {
  const tools = TOOL_NAMES.filter((tool) => parsed[tool]);
