...
export function normalizeScopeFlags(flags, repoArgPath) {
  if (flags.user && flags.repo) {
    throw new ReinError("Choose either --repo or --user, not both.");
  }

  if (repoArgPath && !flags.repo) {
    throw new ReinError("Repository paths must be provided as `rein <command> --repo [path]`.");
  }
