...
import { lookupEvidenceContext } from "./evidence-index.js";
import {
  createGoState,
  goStateExists,
  loadGoState,
...
function buildEvidenceQuery(state, stage, extra = {}) {
  const values = [
    `rein go ${stage}`,
    state.task,
    ...(stage === "plan"
      ? [
          extra.intent,
          extra.desiredOutcome,
          ...(extra.acceptanceCriteria || []),
          ...(extra.constraints || []),
          ...(extra.inScope || []),
          ...(extra.outOfScope || []),
          ...(extra.technicalContext || []),
        ]
      : []),
    ...(stage === "implementation"
      ? [
          "implementation acceptance criteria constraints technical context",
          ...(extra.acceptanceCriteria || []),
          ...(extra.constraints || []),
          ...(extra.technicalContext || []),
        ]
      : []),
    ...(stage === "cleanup"
      ? ["cleanup scope boundaries changed files delete simplify", ...(state.changedFiles || [])]
      : []),
    ...(stage === "review"
      ? [
          "review scope drift debug leftovers test integrity rejected alternatives",
          ...(state.changedFiles || []),
        ]
      : []),
    ...(stage === "verify"
      ? [
          "verify evidence tested not-tested acceptance criteria completion audit",
          ...(state.changedFiles || []),
        ]
      : []),
  ];
...
function evidenceContextForStage(state, stage, extra = {}) {
  return lookupEvidenceContext(buildEvidenceQuery(state, stage, extra), {
    cwd: state.environment.workspaceRoot,
    limit: 6,
  });
}
...
function buildPlanPayload(state, interviewBundle) {
  ...
  return {
    ...
    evidenceContext: evidenceContextForStage(state, "plan", {
      acceptanceCriteria: result.acceptanceCriteria || [],
      constraints: result.constraints || [],
      desiredOutcome: result.desiredOutcome,
...
function buildInstructionPayload(state, stage, extra = {}) {
  if (stage === "implementation") {
    return {
      ...
      evidenceContext: evidenceContextForStage(state, stage, extra),
