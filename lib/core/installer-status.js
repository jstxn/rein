...
function getRepoStatusData(targetRepo, selectedTools = null) {
  ...
  if (tools.includes("cursor")) {
    ...
    for (const skill of REPO_SKILLS) {
      const found = exists(path.join(resolved, ".cursor", "rules", `${skill}.mdc`));
      items.push({ found, kind: "check", label: skill });
    }

    sections.push({ items, label: "Cursor" });
  }
...
function getUserStatusData(selectedTools = null) {
  ...
  const scopePath = (tool, ...parts) => path.join(home, toolRootDir(tool), ...parts);
  ...
  if (tools.includes("cursor")) {
    ...
    for (const skill of REPO_SKILLS) {
      const found = exists(scopePath("cursor", "rules", `${skill}.mdc`));
      items.push({ found, kind: "check", label: skill });
    }
