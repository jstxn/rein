...
const REPO_EVAL_DIRS = ["tasks", "prompts", "reports"];
const REPO_EVAL_FILES = ["run.py", "score.py"];

// `rein init` installs only from the bundled assets inside the REIN package
// root. In a dev checkout, `rein init --repo . --force` means source and
// destination can be the same path, so copy helpers must treat that as a
// no-op rather than deleting bundled assets before copying them back.
...
function missingPackagedAssets() {
  const requiredPaths = [
    ...
    ...REPO_EVAL_DIRS.map((dir) => path.join("rein-evals", dir)),
    ...REPO_EVAL_FILES.map((file) => path.join("rein-evals", file)),
  ];
...
### REIN Artifacts
- Codebase inspection docs from \`deep-inspect\`: \`.rein/codebase/\`
- Interview artifacts from \`deep-interview\`: \`.rein/context/\`, \`.rein/interviews/\`, \`.rein/specs/\`
