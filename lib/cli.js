...
Commands:
  ...
  remove   Uninstall REIN from a repository (interactive)
`;
}
...
async function removeRepo(targetRepo, skipConfirm) {
  ...
  if (exists(reinDir)) {
    ...
    if (skipConfirm) {
      ...
      try {
        const answer = await rl.question(
          ".rein/ contains work artifacts (specs, interviews, codebase maps).\nRemove .rein/ as well? [y/N] ",
        );
    ...
    if (removeArtifacts) {
      ...
      messages.push(`Removed: ${reinDir}`);
    } else {
      messages.push(`Preserved: ${reinDir} (contains work artifacts)`);
    }
  }
