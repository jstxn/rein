...
function resolveInstalledTools(mode, target, selectedTools) {
  const detected =
    mode === "user" ? detectInstalledToolsUser(target) : detectInstalledTools(target);

  if (!selectedTools) {
    return detected;
  }

  return selectedTools.filter((tool) => detected.includes(tool));
}
...
export default class RemoveCommand extends Command {
  ...
  static flags = {
    ...
    cursor: Flags.boolean({
      description: "Only remove Cursor surfaces",
    }),
    repo: Flags.boolean({
  ...
  async run() {
    ...
    const installedTools = resolveInstalledTools(mode, target, tools);

    if (installedTools.length === 0) {
      throw new ReinError("No REIN installation detected. Nothing to remove.");
    }

    if (!flags.yes) {
      if (!isInteractive()) {
        throw new ReinError(
          "Non-interactive remove requires --yes. Re-run with `rein remove --yes`.",
        );
      }
