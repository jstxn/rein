...
## How It Works
- The repo is centered on a small publishable Node package that installs repo-local agent guidance into another repository.
- The shipped runtime surfaces are the protocol doc (`REIN.md`), installer code (`bin/`, `lib/`), and repo-local Codex/Claude/Gemini assets (`.codex/`, `.claude/`, `.gemini/`).
- `bin/rein.js` is the package entrypoint and hands off to `lib/cli.js`.
- `lib/cli.js` copies the harness surfaces into either a target repo or the user-level tool area.
- `.codex/` and `.gemini/` contain skill directories; `.claude/` contains command files that represent the same REIN workflows for Claude Code. Gemini uses shorter skill names at its tool boundary so workspace skills render as `verify` and extension-qualified names render as `rein:verify` rather than `rein:rein-verify`.
...
- The repo favors repo-local agent-native integration over wrapper tooling.
...
- `package.json` defines what will be included in a published package; it includes `.codex/`, `.claude/`, and `.gemini/` assets, but not `.rein/` or `.omc/`.
...
## Findings
- Observed: The published package surface is narrow. `package.json` includes `bin`, `lib`, protocol docs, `.codex` skills, `.claude` commands, `.gemini` skills, and `README.md`.
