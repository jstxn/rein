...
- `.codex/skills/rein-review/SKILL.md`
...
- `rein-review` self-reviews the current diff for unrelated changes, debug leftovers, test weakening, and scope drift before committing.
...
- `rein-interview` and `rein-inspect` are broader orchestration skills, while `rein-triage`, `rein-plan`, `rein-scope`, `rein-review`, `rein-verify`, and `rein-retro` are narrowly scoped procedural checkpoints.
...
## Findings
- Observed: The active repo-local skill files are `rein-interview`, `rein-inspect`, `rein-triage`, `rein-plan`, `rein-scope`, `rein-review`, `rein-verify`, `rein-retro`, and `rein-cleanup`.
